...
export class RestProtocolClient<TRequest, TResponse> {
    //#region  [ constants ]
    private get REQUEST_EVENT (): string{ return `__${this.name}::restprotocol::request__` }
    private get RESPONSE_EVENT (): string { return `__${this.name}::restprotocol::response__` }
    //#endregion
    ...
    //#region [ private ]
    private wait ( millis = 1000 ): Promise<void> {
